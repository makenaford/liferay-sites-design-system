...
/** The light-canvas artwork, same paths. An icon with no file here renders its dark art in both. */
const lightDir = join(root, 'assets', 'glass-icons-light')
...
const keyOf = (dir, file) => relative(dir, file).replace(/\.svg$/, '').split(sep).join('/')

const available = new Map(walk(sourceDir).map((file) => [keyOf(sourceDir, file), file]))
const availableLight = existsSync(lightDir)
  ? new Map(walk(lightDir).map((file) => [keyOf(lightDir, file), file]))
  : new Map()
...
/**
 * One SVG file's contents, as JSX ready to drop inside a `<svg>`.
 *
 * `variant` is `d` or `l` and goes into every namespaced id. The instance's `uid` alone is not enough
 * once an icon carries both artworks: the two exports were made in different Figma sessions but nothing
 * stops them both containing a `paint0_linear_…`, and two identical ids inside one `<svg>` leaves the
 * browser resolving `url(#…)` to whichever came first — which would silently paint the light icon with
 * the dark one's gradient.
 */
function bodyOf(svg, variant) {
  const stripped = svg
    .replace(/^[\s\S]*?<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
    // Figma's background-blur export, and the clip paths only it referenced.
    .replace(/<foreignObject[\s\S]*?<\/foreignObject>/g, '')
    .replace(/<clipPath[^>]*id="bgblur[^"]*"[\s\S]*?<\/clipPath>/g, '')
    .replace(/\sdata-figma-bg-blur-radius="[^"]*"/g, '')
    .trim()

  return toJsx(stripped)
    .replace(/id="([^"]+)"/g, (_, id) => `id={\`\${uid}-${variant}-${id}\`}`)
    .replace(/="url\(#([^)]+)\)"/g, (_, id) => `={\`url(#\${uid}-${variant}-${id})\`}`)
}
...
for (const { path, as } of entries) {
  ...
  const lightFile = availableLight.get(path)
  const inner = bodyOf(svg, 'd')
  const innerLight = lightFile ? bodyOf(readFileSync(lightFile, 'utf8'), 'l') : null

  components.push({ export: name, path, viewBox, inner, innerLight })
}
...
const body = components
  .map((c) => {
    ...
    return `/** Glass icon \`${c.path}\`.${c.innerLight ? '' : ' Dark artwork only — see the note above.'} */
